...
Backend: project skeleton, `User`/`Task` entities, JWT-based register/login/`/api/users/me`, and full ownership-scoped Task CRUD (`/api/tasks`) are implemented. Frontend: Angular skeleton with routing, JWT auth (`AuthService`, `authGuard`, `authInterceptor`, `httpErrorInterceptor` + toast), register/login pages, and a drag-and-drop Kanban task board (`/dashboard`) — glassmorphic "aurora" design system, `@angular/cdk` drag-drop between To do / In progress / Done columns, slide-over create/edit form, custom confirm dialog. Further domains (habits, journal, budget) are not yet built. See the architecture plan for the full build order.
