...
Backend: project skeleton, `User`/`Task`/`BudgetEntry` entities, JWT-based register/login/`/api/users/me`, full ownership-scoped Task CRUD (`/api/tasks`), and full ownership-scoped Budget CRUD + monthly summary (`/api/budget`, `/api/budget/summary`) are implemented. Frontend: Angular skeleton with routing, JWT auth (`AuthService`, `authGuard`, `authInterceptor`, `httpErrorInterceptor` + toast), register/login pages, a dashboard overview (`/dashboard`), a drag-and-drop Kanban task board (`/board`) — glassmorphic "aurora" design system, `@angular/cdk` drag-drop between To do / In progress / Done columns, slide-over create/edit form, custom confirm dialog — and a Budget page (`/budget`) with month navigation, income/expense entries, and a live summary (income / expenses / net). Further domains (habits, journal) are not yet built. See the architecture plan for the full build order.
